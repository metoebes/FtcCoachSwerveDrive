...
public class Robot {
    ...
    static final double ROBOT_WIDTH_HEIGHT_INCHES = (DIAG_DST_BETWEEN_WHEELS_INCHES / Math.sqrt(2.0));
    static final double FIELD_WIDTH_HEIGHT_INCHES =  6 * 12;

    public final static double POT_FORWARD_VOLTAGE = 3.08;
    public final static double POT_ERROR_TOLERANCE = 0.1;
    ...
    public void init(HardwareMap hardwareMap, Config _config, Telemetry _telemetry){
        ...
        config = _config;
        PlaceRobot(
                12 /* positive X is toward the backstage */,
                -(FIELD_WIDTH_HEIGHT_INCHES - (ROBOT_WIDTH_HEIGHT_INCHES /2)) /* negative Y is toward the red side */,
                90  /* 0 degrees points toward backstage */);
        DrawUtilities.drawField(this, telemetry);
    }

    public  void PlaceRobot( double x, double y, double angleInDegrees) {
        pose.positionInInches.x = x;
        pose.positionInInches.y = y;
        pose.heading_radians = AngleUtilities.toRadians(angleInDegrees);
    }
